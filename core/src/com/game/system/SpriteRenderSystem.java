...
public class SpriteRenderSystem extends EntityProcessingSystem {
	@Mapper ComponentMapper<SpriteComponent> sprtc;
	...
	//private Bag<AtlasRegion> regionsByEntity; // collection of regions of every entity to render
	

	public SpriteRenderSystem(OrthographicCamera camera) {
		 super(Aspect.getAspectForAll(SpriteComponent.class, PositionComponent.class, MovementComponent.class,
	...
	protected void process(Entity e) {
		if (sprtc.has(e)) {
			SpriteComponent sprite = sprtc.getSafe(e);
			...
			if (movement.state == MovementComponent.IDLE) {
				animation = new Animation(0.090f, sprite.regions[direction.curdir][0]);
			}else if (movement.state == MovementComponent.MOVING) {
				animation = new Animation(0.090f, sprite.getRegions(direction.curdir));
			}
